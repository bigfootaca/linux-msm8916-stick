 ...
 *	   BigfootACA <bigfoot@classfun.com>
...
#define V4L2_CID_IT6616_AUDIO_SAMPLING_RATE	(V4L2_CID_USER_IT6616_BASE + 1)
#define V4L2_CID_IT6616_AUDIO_PRESENT		(V4L2_CID_USER_IT6616_BASE + 2)

#define to_it6616(sd) container_of(sd, struct it6616, sd)
#define it6616_hdmi_chgbank(regmap,bank) regmap_update_bits(regmap, 0x0F, 0x07, (u8)(bank & 0x07))
...
static u8 regmap_read_u8(struct regmap *regmap, u8 reg)
{
	unsigned int val;
...
static void it6616_hdim_write_table(struct regmap *regmap, struct it6616_reg_set *tdata)
{
	while (tdata->ucAddr != 0xff) {
		if (tdata->andmask == 0xff)
			regmap_write(regmap, tdata->ucAddr, tdata->ucValue);
		else
			regmap_update_bits(regmap, tdata->ucAddr, tdata->andmask, tdata->ucValue);
		tdata++;
	}
}
...
static bool it6616_mipi_tx_get_video_stable(struct it6616 *it6616)
{
	...
	reg09h = regmap_read_u8(mipi, 0x09);
					...
					enum dcs_cmd_name cmd_name)
{
	...
	if (i == size) {
		if (dcs_setting_table[cmd_name].count == 0) {
			dev_warn_ratelimited(dev, "error cmd index: %d count: 0", cmd_name);
...
static void it6616_mipi_tx_get_packet_fire_state(struct it6616 *it6616)
{
	...
	lp_cmd_fifo = regmap_read_u8(mipi, 0x71) & 0x0F;
	link_data_fifo = regmap_read_u8(mipi, 0x72);

	if (lp_cmd_fifo != 0)
		dev_warn_ratelimited(dev,
			"fire low power cmd fail, remain bytes not fire, reg0x71:0x%02x",
			lp_cmd_fifo);
	if (link_data_fifo != 0)
		dev_warn_ratelimited(dev,
			"fire link0 low power data fail, remain %d bytes not fire, reg0x72:0x%02x",
			  ...
			  enum dcs_cmd_name cmd_name)
{
	...
	if (packet_size == UNKNOWN_PACKET) {
		dev_warn(dev, "unknown packet size and check dcs table parameter");
		return;
	}
			...
			enum dcs_cmd_name cmd_name)
{
	...
	regmap_write(mipi, 0x75, dcs_setting_table[cmd_name].cmd);
}
			...
			enum mipi_tx_lp_cmd_header header_select)
{
	...
	if (!header_select) {
		...
		for (i = 0; i < dcs_setting_table[cmd_name].count; i++) {
			regmap_write(mipi, 0x73, dcs_setting_table[cmd_name].para_list[i]);
	...
	for (i = 0; i < sizeof(packet); i++) {
		regmap_write(mipi, 0x73, ((u8 *)(&packet))[i]);
	...
	regmap_write(mipi, 0x73, (u8)long_packet_checksum);
	regmap_write(mipi, 0x73, (u8)(long_packet_checksum >> 8));
	v4l2_dbg(1, debug, sd,
...
short_packet:
	...
	regmap_write(mipi, 0x74, (it6616->mipi_tx_enable_h_fire_packet << 7) | data_count);
}
			...
			int count, enum mipi_tx_lp_cmd_header header_select)
{
	...
	regmap_update_bits(mipi, 0x70, 0x03, 0x03);
	regmap_update_bits(mipi, 0x70, 0x03, 0x00);

	if (enable_force_lp_mode) {
		regmap_update_bits(mipi, 0x05, 0x16, 0x16);
		regmap_update_bits(mipi, 0x05, 0x16, 0x10);
		regmap_update_bits(mipi, 0x70, 0x04, 0x04);
	}

	regmap_write(mipi, 0x3D, 0x00);
	regmap_write(mipi, 0x3E, enable_force_lp_mode ? 0x00 : 0x10);
	regmap_write(mipi, 0x3F, enable_force_lp_mode ? 0x30 : 0x90);

	for (i = start; i < start + count; i++) {
		dev_dbg(dev, "cmd:%d tx reg09:0x%02x\n", i, regmap_read_u8(mipi, 0x09));
		if (i >= dcs_table_size)
		...
		if (data_count > lp_cmd_fifo_size[0]) {
			dev_err(dev, "error lp cmd: %d, exceed cmd fifo\n", i);
			continue;
		}
...
complete_write_dcs:
	...
	if (enable_force_lp_mode) {
		regmap_update_bits(mipi, 0x70, 0x04, 0x00);
		regmap_update_bits(mipi, 0x05, 0x16, 0x00);
	}
...
static void it6616_enter_bus_turn_around(struct it6616 *it6616)
{
	...
	regmap_write(mipi, 0x3E, 0x10);
	regmap_write(mipi, 0x3F, 0x90);

	regmap_write(mipi, 0x74, it6616->mipi_tx_enable_h_fire_packet << 7);
	regmap_write(mipi, 0x75, LP_CMD_BTA);
		...
		int dcs_table_size, enum dcs_cmd_name cmd_name, u8 *buffer)
{
	...
	msleep(20);
	link0_data_count = regmap_read_u8(mipi, 0x7A);

	for (i = 0; i < link0_data_count; i++)
		buffer[i] = regmap_read_u8(mipi, 0x79);
}
...
static void it6616_mipitx_setup_dsi(struct it6616 *it6616)
{
	...
	regmap_update_bits(mipi, 0x28, 0x20, bus->tx_sel_line_start << 5);
	...
	//dsi setting
	regmap_update_bits(mipi, 0x5e, 0x03, (bus->tx_vlpm_length >> 8) & 0x03);
	regmap_update_bits(mipi, 0x5d, 0xff, bus->tx_vlpm_length & 0xFF);
	regmap_update_bits(mipi, 0x5e, 0x0c, (bus->tx_hlpm_length & 0x300) >> 6);
	regmap_update_bits(mipi, 0x5f, 0xff, bus->tx_hlpm_length & 0xFF);
	regmap_update_bits(mipi, 0x5e, 0x10, bus->tx_enable_h_enter_lpm << 4);
	regmap_update_bits(mipi, 0x6a, 0xff, bus->p2m_delay.tx_dsi_vsync_delay);
	regmap_update_bits(mipi, 0x6c, 0xff, mp_hs_pretime);
	regmap_update_bits(mipi, 0x6d, 0xff, mp_hs_endtime);
	regmap_update_bits(mipi, 0x5c, 0x03, (MIPI_TX_ENABLE_DSI_EOTP_PACKET << 1) |
			MIPI_TX_ENABLE_DSI_SYNC_EVENT);
	regmap_update_bits(mipi, 0x5c, 0xf0, ((mp_vid_type & 0x30) << 2) |
			((mp_vid_type & 0x3) << 4));
	regmap_update_bits(mipi, 0x60, 0x0f, p2m_time_mul);
	regmap_update_bits(mipi, 0x61, 0x03, p2m_time_div);
...
static void it6616_mipitx_setup_csi(struct it6616 *it6616)//set_mptx
{
	...
	interlace = (regmap_read_u8(hdmi, 0x98) & 0x02) >> 1;
	...
	regmap_write(mipi, 0x1F, mp_hs_endtime);
	regmap_write(mipi, 0x22, mp_hs_pretime);
	regmap_write(mipi, 0x24, 0x20);
	regmap_write(mipi, 0x25, bus->p2m_delay.tx_csi_p2m_delay);
	regmap_update_bits(mipi, 0x26, 0x20, (en_fs_fr_num << 5));
	regmap_write(mipi, 0x27, 0x02);
	regmap_write(mipi, 0x20, mp_vid_type);
	regmap_write(mipi, 0x23, reg23);
...
static void it6616_mipi_tx_non_continuous_clock_setup(struct it6616 *it6616)
{
	...
	regmap_update_bits(mipi, 0x44, 0x01, 0x00);

	dev_dbg(dev, "set mipi tx non continuous clock\n");
}
...
adjust_lane_count:
	dev_info(dev, "MIPI interface: %s\n", bus_type_index ? "MIPI_DSI" : "MIPI_CSI");
	dev_info(dev, "MIPI color space: %s\n", (color_space_name_index != 0xFF) ?
		mipi_color_space_name[bus_type_index][color_space_name_index] : "not found");
	dev_dbg(dev, "lan_num: %d, swap_pn: %d\n", it6616->mipi.lane_cnt, bus->swap_pn);
	dev_dbg(dev, "swap_lan: %d, pclk_inv: %d\n", bus->swap_lan, bus->pclk_inv);
	dev_dbg(dev, "mclk_inv: %d, lpx_num: %d\n", bus->mclk_inv, bus->lpx_num);
	...
	dev_dbg(dev, "prediv: 0x%02x, plldiv: 0x%02x\n", mprediv, mplldiv);
	...
	if (it6616->mipi_tx_enable_auto_adjust_lane_count) {
		if (mclk_MHz < MIPI_TX_LANE_ADJUST_THRESHOLD) {
			...
			dev_dbg(dev, "mclk < %d MHz, adjust to lan_num: %d\n",
	...
	regmap_update_bits(mipi, 0x28, 0x0c, (bus->swap_pn << 3) | (bus->swap_lan << 2));
	regmap_update_bits(mipi, 0x10, BIT(2), bus->pclk_inv << 2);

	switch (regmap_read_u8(mipi, 0x04)) {
	case 0xC0:
		regmap_update_bits(mipi, 0x10, BIT(1), bus->mclk_inv << 1);
		break;
	default:
		regmap_update_bits(mipi, 0x11, BIT(3), bus->mclk_inv << 3);
	}

	regmap_update_bits(mipi, 0x8c, 0x40, 0x00);
	regmap_update_bits(mipi, 0x47, 0xf0, bus->mipi_tx_hs_prepare << 4);
	regmap_update_bits(mipi, 0x44, 0x04, bus->tx_enable_hs_pre_1T << 2);
	regmap_update_bits(mipi, 0x21, 0x30, (it6616->mipi.lane_cnt - 1) << 4);
	...
	if ((pclk < (10 * (mprediv + 1))) || (pclk > (100 * (mprediv + 1))))
		dev_err(dev, "MPTX PHY setting wrong, need to reset parameter for TXPHY\n");
...
static void it6616_mipi_tx_get_support_format(struct it6616 *it6616)
{
	...
	if (it6616->mipi.bus_type == MIPI_CSI) {
		...
		if (color_space_name_index != 0xFF)
			dev_dbg(dev, "will set %s\n",
				mipi_color_space_name[bus_type_index][color_space_name_index]);
		else
			dev_warn(dev, "error not find match color space\n");
	} else {
		if (regmap_read_u8(mipi, 0x04) == 0xC0) {
...
static __maybe_unused void it6616_mipi_tx_rk_fix_first_bit_issue(struct it6616 *it6616)
{
	...
	regmap_write(mipi, 0x05, 0x00);
	regmap_update_bits(mipi, 0x28, 0x08, 0x08);
	regmap_update_bits(mipi, 0x70, 0x04, 0x04);
	regmap_update_bits(mipi, 0x28, 0x08, 0x00);
	regmap_update_bits(mipi, 0x70, 0x04, 0x00);
	regmap_write(mipi, 0x05, 0x36);
}
...
static void it6616_mipitx_initial(struct it6616 *it6616)
{
	...
	regmap_update_bits(mipi, 0x05, 0x09, 0x09);
	regmap_write(mipi, 0x05, 0x36);

	regmap_update_bits(mipi, 0x2A, 0x3c, 0x00);
	regmap_update_bits(mipi, 0x3c, 0x20, 0x20);
	regmap_update_bits(mipi, 0x6b, 0x01, it6616->mipi.bus_type);
	regmap_update_bits(mipi, 0x10, 0x80, bus->tx_bypass << 7);
	regmap_update_bits(mipi, 0xc1, 0x03, 0x03);
	regmap_update_bits(mipi, 0xa8, 0x01, 0x00);
	regmap_update_bits(mipi, 0x45, 0x0f, bus->lpx_num);
	...
	dev_dbg(dev, "mipi_initial chip: 0x%02x\n", regmap_read_u8(mipi, 0x04));
}
...
static bool it6616_hdmi_is_5v_on(struct it6616 *it6616)
{
	...
	if (regmap_read_u8(hdmi, 0x13) & 0x01)
		return true;
...
static bool it6616_hdmi_is_clock_stable(struct it6616 *it6616)
{
	...
	if (regmap_read_u8(hdmi, 0x13) & 0x10)
		return true;
...
static __maybe_unused bool it6616_hdmi_is_symbol_locked(struct it6616 *it6616)
{
	...
	reg14 = regmap_read_u8(hdmi, 0x14);
	if ((reg14 & 0x38) == 0x38)
		return true;
...
static bool it6616_hdmi_is_scdt_on(struct it6616 *it6616)
{
	...
	if (regmap_read_u8(hdmi, 0x19) & BIT(7))
		return true;
...
static void it6616_hdmi_edid_ram_get(struct it6616 *it6616, u8 *buf)
{
	...
	regmap_write(hdmi, 0x4B, (I2C_ADR_EDID | 0x01));
	regmap_bulk_read(edid, 0, buf, 256);
	regmap_write(hdmi, 0x4B, (I2C_ADR_EDID));
}
...
static void it6616_hdmi_edid_ram_update_chksum(struct it6616 *it6616)
{
	...
	regmap_write(hdmi, 0xC9, sum);

	// cal block 1 sum
	sum = 0;
	offset = regmap_read_u8(hdmi, 0xC6);
	for (i = 128; i < 128 + 127; i++)
		sum += it6616->edid_data[i];

	sum -= it6616->edid_data[offset];
	sum -= it6616->edid_data[offset + 1];
	sum += regmap_read_u8(hdmi, 0xC7);
	sum += regmap_read_u8(hdmi, 0xC8);
	sum = (0x100 - sum) & 0xFF;

	regmap_write(hdmi, 0xCA, sum);
}
...
static void it6616_hdmi_edid_ram_init(struct it6616 *it6616)
{
	...
	regmap_write(hdmi, 0x4B, (I2C_ADR_EDID | 0x01));
	regmap_bulk_write(edid, 0, it6616->edid_data, it6616->edid_len);
	regmap_write(hdmi, 0x4B, (I2C_ADR_EDID));
	...
	if (addr != CEC_PHYS_ADDR_INVALID) {
		regmap_write(hdmi, 0xC6, (u8)phy_addr_off); // VSDB start address
		regmap_write(hdmi, 0xC7, (addr >> 8) & 0xFF); // addr AB
		regmap_write(hdmi, 0xC8, addr & 0xFF); // addr CD
	}

	// recalculate block0/block1 checksum
	it6616_hdmi_edid_ram_update_chksum(it6616);

	regmap_update_bits(hdmi, 0xC5, 0x10, 0x10);
	msleep(20);
	regmap_update_bits(hdmi, 0xC5, 0x10, 0x00);
}
...
static void it6616_hdmi_video_reset(struct it6616 *it6616)
{
	...
	regmap_update_bits(hdmi, 0x22, BIT(0), BIT(0));
	msleep(20);
	regmap_update_bits(hdmi, 0x22, BIT(0), 0x00);
	regmap_update_bits(hdmi, 0x10, BIT(1), BIT(1)); // clear vidstable change INT
	regmap_update_bits(hdmi, 0x12, BIT(7), BIT(7)); // clear vidstable change INT
}
...
static void it6616_hdmi_edid_ram_enable(struct it6616 *it6616, u8 enabled)
{
	...
	if (enabled)
		regmap_update_bits(hdmi, 0xC5, 0x3F, 0x02);
	else
		regmap_update_bits(hdmi, 0xC5, 0x3F, 0x13);
}
...
static void it6616_hdmi_rx_get_video_info(struct it6616 *it6616)
{
	...
	interlaced = (regmap_read_u8(hdmi, 0x98) & 0x02) >> 1;

	h_total = ((regmap_read_u8(hdmi, 0x9C) & 0x3F) << 8) + regmap_read_u8(hdmi, 0x9B);
	h_active = ((regmap_read_u8(hdmi, 0x9E) & 0x3F) << 8) + regmap_read_u8(hdmi, 0x9D);
	h_front_porch = ((regmap_read_u8(hdmi, 0xA1) & 0xF0) << 4) + regmap_read_u8(hdmi, 0xA0);
	h_sync_w = ((regmap_read_u8(hdmi, 0xA1) & 0x01) << 8) + regmap_read_u8(hdmi, 0x9F);

	v_total = ((regmap_read_u8(hdmi, 0xA3) & 0x3F) << 8) + regmap_read_u8(hdmi, 0xA2);
	v_active = ((regmap_read_u8(hdmi, 0xA5) & 0x3F) << 8) + regmap_read_u8(hdmi, 0xA4);
	v_front_porch = ((regmap_read_u8(hdmi, 0xA8) & 0xF0) << 4) + regmap_read_u8(hdmi, 0xA7);
	v_sync_w = ((regmap_read_u8(hdmi, 0xA8) & 0x01) << 8) + regmap_read_u8(hdmi, 0xA6);

	h_sync_pol = (regmap_read_u8(hdmi, 0xAA) & BIT(5)) >> 5;
	v_sync_pol = (regmap_read_u8(hdmi, 0xAA) & BIT(6)) >> 6;
...
static void it6616_hdmi_hpd_output(struct it6616 *it6616, u8 hpd)
{
	...
	if (hpd)
		regmap_update_bits(hdmi, 0xAB, 0xC0, 0xC0); // SET PORT0 HPD HIGH
	else
		regmap_update_bits(hdmi, 0xAB, 0xC0, 0x40); // SET PORT0 HPD LOW
...
static void it6616_hdmi_hdcp_reset(struct it6616 *it6616)
{
	...
	regmap_update_bits(hdmi, 0x23, 0x02, 0x02);
	regmap_update_bits(hdmi, 0x23, 0x02, 0x00);
}
...
static bool it6616_hdmi_get_hdcp_status(struct it6616 *it6616)
{
	...
	return regmap_read_u8(hdmi, 0xCF) & BIT(5);
}
...
static enum av_mute_state it6616_hdmi_rx_get_av_mute_state(struct it6616 *it6616)
{
	...
	it6616_hdmi_chgbank(hdmi, 0);
	av_mute_state = !!(regmap_read_u8(hdmi, 0xAA) & BIT(3));
...
static void it6616_hdmi_rx_set_av_mute(struct it6616 *it6616, enum av_mute_state mute_state)
{
	...
	switch (mute_state) {
	case AV_MUTE_OFF:
		regmap_update_bits(hdmi, 0x4F, BIT(5), 0x00);
		...
		break;
	case AV_MUTE_ON:
		regmap_update_bits(hdmi, 0x4F, BIT(5), BIT(5));
...
static void it6616_hdmi_update_rs(struct it6616 *it6616, u8 level)
{
	...
	it6616_hdmi_chgbank(hdmi, 3);
	regmap_update_bits(hdmi, 0x26, 0x20, 0x00);
	regmap_write(hdmi, 0x27, rs_val);
	regmap_write(hdmi, 0x28, rs_val);
	regmap_write(hdmi, 0x29, rs_val);
	it6616_hdmi_chgbank(hdmi, 0);
}
...
static void it6616_mipi_tx_calc_rclk(struct it6616 *it6616)
{
	...
	for (i = 0; i < retry; i++) {
		regmap_update_bits(mipi, 0xE0, 0x80, 0x80); // Enable RCLK 100ms count
		msleep(100);
		regmap_update_bits(mipi, 0xE0, 0x80, 0x00); // Disable RCLK 100ms count

		ul100msCNT = regmap_read_u8(mipi, 0xE3);
		ul100msCNT = ((ul100msCNT << 8) | (regmap_read_u8(mipi, 0xE2)));
		ul100msCNT = ((ul100msCNT << 8) | (regmap_read_u8(mipi, 0xE1)));
...
static void it6616_mipi_tx_calc_mclk(struct it6616 *it6616)
{
	...
	for (i = 0; i < retry; i++) {
		regmap_update_bits(mipi, 0xE7, 0x80, 0x80);
		msleep(20);
		regmap_update_bits(mipi, 0xE7, 0x80, 0x00);

		ulCNT = regmap_read_u8(mipi, 0xE7) & 0x0F;
		ulCNT = (regmap_read_u8(mipi, 0xE6) | (ulCNT << 8));
	...
	dev_dbg(dev, "mipi mclk = %lu.%lu MHz\n", tx_mclk / 1000, tx_mclk % 1000);
}
...
static void it6616_mipi_tx_calc_pclk(struct it6616 *it6616)
{
	...
	for (i = 0; i < retry; i++) {
		regmap_update_bits(mipi, 0xE5, 0x80, 0x80);
		msleep(20);
		regmap_update_bits(mipi, 0xE5, 0x80, 0x00);

		ulCNT = regmap_read_u8(mipi, 0xE5) & 0x0F;
		ulCNT = regmap_read_u8(mipi, 0xE4) + (ulCNT << 8);
...
static u32 it6616_hdmi_rx_calc_rclk(struct it6616 *it6616)
{
	...
	for (i = 0; i < retry; i++) {
		regmap_update_bits(hdmi, 0x58, 0x80, 0x80);
		msleep(100);
		regmap_update_bits(hdmi, 0x58, 0x80, 0x00);

		rddata = regmap_read_u8(hdmi, 0x59);
		rddata += (regmap_read_u8(hdmi, 0x5A) << 8);
		rddata += (regmap_read_u8(hdmi, 0x5B) << 16);
	...
	regmap_update_bits(hdmi, 0x1E, 0x3F, t1usint & 0x3F);
	regmap_write(hdmi, 0x1F, t1usflt);
...
__recal:
	for (i = 0; i < retry; i++) {
		msleep(20);
		regmap_update_bits(hdmi, 0x9A, BIT(7), 0x00);
		rddata = ((u32)(regmap_read_u8(hdmi, 0x9A) & 0x03) << 8) +
							regmap_read_u8(hdmi, 0x99);
		regmap_update_bits(hdmi, 0x9A, BIT(7), BIT(7));
		sump += rddata;
	}
	...
	if (sump) {
		...
		dev_dbg(dev, "PCLK: %u.%03u MHz\n", pclk / 1000, pclk % 1000);
		it6616->vinfo.pclk = pclk;
	} else {
		dev_warn_ratelimited(dev, "hdmi_rx_calc_pclk: sump is 0\n");
		goto __recal;
	}
...
static void it6616_hdmi_rx_calc_tmds_clk(struct it6616 *it6616, u8 count)
{
	...
	for (i = 0; i < count; i++) {
		msleep(20);
		rddata = regmap_read_u8(hdmi, 0x48) + 1;
		sumt += rddata;
	}

	if (sumt) {
		rddata = regmap_read_u8(hdmi, 0x43) & 0xE0;
		...
		dev_warn_ratelimited(dev, "hdmi_rx_calc_tmds_clk: sumt is 0\n");
	}
}
...
static int it6616_hdmi_update_avi_infoframe(struct it6616 *it6616)
{
	...
	avi_packet[1] = regmap_read_u8(hdmi, REG_RX_AVI_HB1);// version
	avi_packet[2] = regmap_read_u8(hdmi, REG_RX_AVI_HB2);// version
...
static void it6616_hdmi_rx_setup_csc(struct it6616 *it6616)
{
	...
	regmap_update_bits(hdmi, 0x6E, 0xF7, reg6e);

	regmap_update_bits(hdmi, 0x6C, 0x03, csc_select);
	...
	case HDMI_COLORSPACE_RGB:
		...
		break;
	default:
		dev_warn(dev, "unknown input color space: %x\n", color_in);
		break;
	}
	...
	case HDMI_COLORSPACE_RGB:
		...
		break;
	default:
		dev_warn(dev, "unknown output color space: %x\n", color_out);
		break;
	}
	...
	regmap_update_bits(hdmi, 0x6B, 0x3F, reg6b);
}
...
static void it6616_hdmi_rx_reset_audio_logic(struct it6616 *it6616)
{
	...
	regmap_update_bits(hdmi, 0x22, BIT(1), BIT(1)); // audio reset
	msleep(20);
	regmap_update_bits(hdmi, 0x22, BIT(1), 0x00);

	// RegFS_Set[5:0] : Software set sampling frequency R/W
	temp = regmap_read_u8(hdmi, 0x8A);
	regmap_write(hdmi, 0x8A, temp);
	regmap_write(hdmi, 0x8A, temp);
	regmap_write(hdmi, 0x8A, temp);
	regmap_write(hdmi, 0x8A, temp);
}
...
static void it6616_hdmi_rx_audio_setup_i2s_justified(struct it6616 *it6616, u8 i2s_justified)
{
	...
	regmap_update_bits(hdmi, 0x0F, 0x03, 0x00);
	regmap_update_bits(hdmi, 0x82, 0x03, i2s_justified);
}
...
static void it6616_hdmi_tx_audio_setup(struct it6616 *it6616)
{
	...
	regmap_update_bits(hdmi, 0x86, BIT(0), BIT(0));
	it6616_hdmi_rx_audio_setup_i2s_justified(it6616, FROM_CONFIG);
	it6616_hdmi_chgbank(hdmi, 2);
	it6616->ainfo.n = ((u32)regmap_read_u8(hdmi, 0xBE) << 12) +
			((u32)regmap_read_u8(hdmi, 0xBF) << 4) +
			((u32)regmap_read_u8(hdmi, 0xC0) & 0x0F);
	it6616->ainfo.cts = regmap_read_u8(hdmi, 0xC0) >> 4;
	it6616->ainfo.cts |= ((u32)regmap_read_u8(hdmi, 0xC1)) << 12;
	it6616->ainfo.cts |= ((u32)regmap_read_u8(hdmi, 0xC2)) << 4;
	it6616_hdmi_chgbank(hdmi, 0);

	if (it6616->ainfo.cts == 0) {
		dev_warn(dev, "hdmi_tx_audio_setup: cts: %u\n", it6616->ainfo.cts);
		return;
	}
	...
	it6616->ainfo.channel_status = ((regmap_read_u8(hdmi, 0xB5) & 0xC0) >> 2) +
					(regmap_read_u8(hdmi, 0xB5) & 0x0F);
	cts_128 = 128 * it6616->ainfo.cts;
	sum = it6616->ainfo.n * it6616->vinfo.TMDSCLK;
	it6616->ainfo.sample_freq = sum / cts_128;

	dev_dbg(dev, "n = %u cts = %u\n", it6616->ainfo.n, it6616->ainfo.cts);
	dev_dbg(dev, "tmds clock = %d kHz\n", it6616->vinfo.TMDSCLK);
	dev_dbg(dev, "Audio_CH_Status[24:27 - 30:31][bit0~bit5] = 0x%02x\n",
		it6616->ainfo.channel_status);
	dev_dbg(dev, "sw clac sampling frequency = %d.%d kHz\n",
	...
	dev_dbg(dev, "Sampling_Frequency value 0x%02x", it6616->ainfo.force_sample_freq);

	if (it6616->ainfo.channel_status == it6616->ainfo.force_sample_freq) {
		dev_dbg(dev, "channel_status == force_sample_freq\n");
		if (regmap_read_u8(hdmi, 0x81) & BIT(6)) {
			// RegForce_FS : 0: Disable Force Audio FS mode
			regmap_update_bits(hdmi, 0x81, BIT(6), 0x00);
	...
	if (it6616->audio_sampling_freq_error_count > MAX_AUDIO_SAMPLING_FREQ_ERROR_COUNT) {
		regmap_update_bits(hdmi, 0x81, BIT(6), BIT(6));	// RegForce_FS : Force Audio FS mode
		// RegFS_Set[5:0] : Software set sampling frequency
		regmap_update_bits(hdmi, 0x8A, 0x3F, it6616->ainfo.force_sample_freq);

#if defined(Enable_Audio_Compatibility) && (Enable_Audio_Compatibility == 1)
		if (it6616->ainfo.sample_freq <= 182) {
			regmap_update_bits(hdmi, 0x89, 0x0C, 0x04);
			regmap_update_bits(hdmi, 0x86, 0x0C, 0x0C);
		} else {
			regmap_update_bits(hdmi, 0x89, 0x0C, 0x0C);
			regmap_update_bits(hdmi, 0x86, 0x0C, 0x04);
		}
#endif
		it6616->audio_sampling_freq_error_count = 0;
		it6616_hdmi_rx_reset_audio_logic(it6616);
...
static void it6616_hdmi_tx_audio_output_enable(struct it6616 *it6616, u8 output_interface)
{
	...
	case AUDIO_OFF:
		dev_dbg(dev, "audio off\n");
		regmap_write(hdmi, 0xC7, 0x7F); // SPDIF/I2S tri-state on
		break;

	case AUDIO_I2S:
	case AUDIO_SPDIF:
		dev_dbg(dev, "enable audio output\n");
		regmap_write(hdmi, 0xC7, 0x00); // SPDIF/I2S tri-state off
		break;
	}
...
static void it6616_hdmi_audio_mute_clear(struct it6616 *it6616)
{
	...
	regmap_update_bits(hdmi, 0x8C, BIT(4), BIT(4)); // set RegHWMuteClr
	regmap_update_bits(hdmi, 0x8C, BIT(4), 0x00); // clear RegHWMuteClr for clear H/W Mute
}
...
static void it6616_hdmi_initial(struct it6616 *it6616)
{
	...
	regmap_update_bits(hdmi, 0x67, BIT(7), it6616->hdmi_rx_disable_pixel_repeat << 7);
	regmap_update_bits(hdmi, 0x69, BIT(6) | BIT(5),
			it6616->hdmi_rx_video_stable_condition << 5);
	dev_dbg(dev,
		"set hdmi rx video stable condition, reg0x69[6:5], 0x%02x\n",
		regmap_read_u8(hdmi, 0x69));
}
...
static void it6616_hdmi_irq_color_depth(struct it6616 *it6616)
{
	...
	input_color_depth = (regmap_read_u8(hdmi, 0x98) >> 4) & 0x0F;
...
static void it6616_mipitx_irq(struct it6616 *it6616)
{
	...
	reg09h = regmap_read_u8(mipi, 0x09);
	reg0ah = regmap_read_u8(mipi, 0x0A);
	reg0bh = regmap_read_u8(mipi, 0x0B);

	regmap_write(mipi, 0x0A, reg0ah);
	regmap_write(mipi, 0x0B, reg0bh);

	if (reg0bh & 0x10) {
		it6616->mipi_tx_video_stable = it6616_mipi_tx_get_video_stable(it6616);
		dev_dbg(dev, "mipi tx Video Stable Change ...\n");
		dev_dbg(dev, "mipi tx reg09 = 0x%02x, video %sstable\n",
	...
	if (reg0ah & 0x70) {
		if (reg0ah & 0x20)
			dev_err(dev, "MIPI Byte mismatch error\n");
		if (reg0ah & 0x40)
			dev_err(dev, "MIPI P2M FIFO error\n");
	}
}
...
static void it6616_hdmi_irq(struct it6616 *it6616)
{
	...
	reg05h = regmap_read_u8(hdmi, 0x05);
	regmap_write(hdmi, 0x05, reg05h);
	reg06h = regmap_read_u8(hdmi, 0x06);
	regmap_write(hdmi, 0x06, reg06h);
	Reg08h = regmap_read_u8(hdmi, 0x08);
	regmap_write(hdmi, 0x08, Reg08h);
	Reg09h = regmap_read_u8(hdmi, 0x09);
	regmap_write(hdmi, 0x09, Reg09h);

	Reg10h = regmap_read_u8(hdmi, 0x10);
	regmap_write(hdmi, 0x10, Reg10h);

	Reg11h = regmap_read_u8(hdmi, 0x11);
	regmap_write(hdmi, 0x11, Reg11h);

	Reg12h = regmap_read_u8(hdmi, 0x12);
	regmap_write(hdmi, 0x12, Reg12h & 0x7F);

	Reg13h = regmap_read_u8(hdmi, 0x13);
	Reg14h = regmap_read_u8(hdmi, 0x14);
	Reg15h = regmap_read_u8(hdmi, 0x15);

	if (reg05h != 0x00) {
		if (reg05h & 0x10)
			dev_dbg(dev, "HDMI mode chg\n");
		if (reg05h & 0x20) {
			dev_err(dev, "HDMI ECC error\n");
			it6616_hdmi_hdcp_reset(it6616);
		}
		if (reg05h & 0x40)
			dev_err(dev, "HDMI Deskew error\n");
		if (reg05h & 0x80)
			dev_err(dev, "HDMI H2VSkew fail\n");
		if (reg05h & 0x04) {
			dev_dbg(dev, "HDMI input clock change detect\n");
			if (it6616_hdmi_is_clock_stable(it6616))
				dev_dbg(dev, "HDMI clock stable\n");
			else
				dev_err(dev, "HDMI clock untable\n");
		}

		if (reg05h & 0x02)
			dev_dbg(dev, "HDMI RX CKOn detect\n");

		if (reg05h & 0x01) {
			dev_dbg(dev, "HDMI 5V state change INT\n");
			it6616_hdmi_hdcp_reset(it6616);
	...
	if (reg06h != 0x00) {
		if (reg06h & 0x80)
			dev_err(dev, "HDMI FSM Error\n");
		if (reg06h & 0x40)
			dev_err(dev, "HDMI CH2 Symbol lock Rst\n");
		if (reg06h & 0x20)
			dev_err(dev, "HDMI CH1 Symbol lock Rst\n");
		if (reg06h & 0x10)
			dev_err(dev, "HDMI CH0 Symbol lock Rst\n");
		if (reg06h & 0x08)
			dev_err(dev, "HDMI CH2 CDR FIFO Aut0-Rst\n");
		if (reg06h & 0x04)
			dev_err(dev, "HDMI CH1 CDR FIFO Aut0-Rst\n");
		if (reg06h & 0x02)
			dev_err(dev, "HDMI CH0 CDR FIFO Aut0-Rst\n");
		if (reg06h & 0x01)
			dev_info(dev, "HDMI Symbol Lock State Change\n");
	}

	if (Reg09h != 0x00) {
		if (Reg09h & 0x01)
			dev_dbg(dev, "HDCP Authentication Start\n");
		if (Reg09h & 0x02) {
			dev_dbg(dev, "HDCP Authentication Done\n");
			it6616->hdmi_rx_hdcp_state = true;
		}
		if (Reg09h & 0x04) {
			it6616->hdmi_rx_hdcp_state = it6616_get_hdcp_status(it6616);
			dev_dbg(dev, "HDCP Encryption change interrupt\n");
			dev_dbg(dev, "HDCP Encryption %s\n",
				it6616->hdmi_rx_hdcp_state ? "ON" : "OFF");
		}
		if (Reg09h & 0x08) {
			dev_dbg(dev, "HDCP Off\n");
	...
	if (Reg12h != 0x00) {
		if (Reg12h & BIT(7)) {
			Reg1Ah = regmap_read_u8(hdmi, 0x1A);
			Reg1Bh = regmap_read_u8(hdmi, 0x1B) & 0x07;
			dev_dbg(dev, "Video Parameters Change\n");
			dev_dbg(dev, "VidParaChange_Sts=Reg1Bh=0x%02X Reg1Ah=0x%02X\n",
			...
			regmap_write(hdmi, 0x12, 0x80);
		}

		if (Reg12h & BIT(6))
			dev_dbg(dev, "HDMI 3D audio Valie Change\n");
		if (Reg12h & BIT(5))
			dev_dbg(dev, "HDMI DRM pkt Change\n");
		if (Reg12h & 0x10)
			dev_dbg(dev, "HDMI New Audio PKT Received\n");
		if (Reg12h & 0x08)
			dev_dbg(dev, "HDMI New ACP PKT Received\n");
		if (Reg12h & 0x04)
			dev_dbg(dev, "HDMI New SPD PKT Received\n");
		if (Reg12h & 0x02)
			dev_dbg(dev, "HDMI New MPEG InfoFrame Received\n");
		if (Reg12h & 0x01) {
			dev_dbg(dev, "HDMI New AVI InfoFrame Received\n");
	...
	if (Reg10h != 0x00) {
		if (Reg10h & 0x80) {
			dev_err(dev, "HDMI Audio FIFO Error\n");
			it6616_hdmi_rx_audio_process(it6616);
		}
		if (Reg10h & 0x40)
			dev_dbg(dev, "HDMI Audio Auto Mute\n");
		// todo: how about on/off flag at the same time ?
		if ((Reg10h & 0x20)) {
			dev_dbg(dev, "HDMI PKT Left Mute\n");
			it6616_hdmi_rx_set_av_mute(it6616, AV_MUTE_OFF);
		}
		if ((Reg10h & 0x10)) {
			dev_dbg(dev, "HDMI Set Mute PKT Received\n");
			it6616_hdmi_rx_set_av_mute(it6616, AV_MUTE_ON);
		}
		if (Reg10h & 0x08)
			dev_dbg(dev, "HDMI Timer Counter Interrupt\n");
		if (Reg10h & 0x04)
			dev_dbg(dev, "HDMI Video Mode Changed\n");
		if (Reg10h & 0x02) {
			it6616->hdmi_rx_video_stable = it6616_hdmi_is_scdt_on(it6616);
			dev_dbg(dev, "HDMI SCDT %s", it6616->hdmi_rx_video_stable ? "ON" : "OFF");
		...
		if (Reg10h & 0x01)
			dev_dbg(dev, "HDMI Video Abnormal Interrupt\n");
	}

	if (Reg11h != 0x00) {
		if (Reg11h & BIT(5))
			dev_dbg(dev, "HDMI No Audio InfoFrame Received\n");
		if (Reg11h & BIT(4))
			dev_dbg(dev, "HDMI No AVI InfoFrame Received\n");
		if (Reg11h & BIT(3)) {
			dev_dbg(dev, "HDMI CD Detect\n");
			it6616_hdmi_irq_color_depth(it6616);
		}
		if (Reg11h & BIT(1))
			regmap_write(hdmi, 0x11, BIT(1));
		if (Reg11h & BIT(0))
			regmap_write(hdmi, 0x11, BIT(0));
	}

	if (Reg13h != 0x00) {
		if (Reg13h & BIT(0))
			dev_dbg(dev, "HDMI Port 0 power 5V detect\n");
		if (Reg13h & BIT(1))
			dev_dbg(dev, "HDMI Port 0 HDMI mode\n");
		else
			dev_dbg(dev, "HDMI Port 0 DVI mode\n");
	}

	if (Reg14h != 0x00) {
		if (Reg14h & BIT(0))
			dev_dbg(dev, "HDMI Port 0 IPLL clock is higher than 100MHz\n");
	}

	if (Reg15h != 0x00) {
		if (Reg15h & BIT(6))
			dev_dbg(dev, "HDMI Port 0 EDID Idle\n");
		else
			dev_dbg(dev, "HDMI Port 0 EDID active\n");
	}
}
...
static int it6616_get_chip_id(struct it6616 *it6616)
{
	...
	for (i = 0; i < sizeof(chip_id); i++) {
		ret = regmap_read_u8(hdmi, i);

		if (ret != chip_id[i]) {
			dev_err(dev, "not it6616 chip_id[%d]: 0x%02x\n", i, ret);
			return -ENODEV;
		}
	}
...
{
	struct regmap *hdmi = it6616->hdmi_regmap;
	enum av_mute_state mute_state_config = !!(regmap_read_u8(hdmi, 0x4F) & BIT(5));
			...
			struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&it6616->confctl_mutex);
	it6616_hdmi_edid_ram_get(it6616, buf);
	mutex_unlock(&it6616->confctl_mutex);
	...
	struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (ret) {
		dev_dbg(dev, "addr = %2.2X\n", addr);
		dev_dbg(dev, "val = %2.2X\n", val);
				...
				u32 height)
{
	...
	if (i == ARRAY_SIZE(supported_modes)) {
		v4l2_err(sd, "rcv does not support res %dx%d\n",
				width, height);
		return false;
	} else {
		return true;
				     ...
				     struct v4l2_dv_timings *timings)
{
	...
	if (!it6616_rcv_supported_res(sd, bt->width, bt->height)) {
		...
		v4l2_err(sd, "rcv err res, assume no signal\n");
		return -EINVAL;
	}
	...
	v4l2_dbg(1, debug, sd, "act:%dx%d, total:%dx%d, pixclk:%lld, fps:%d\n",
...
static void it6616_format_change(struct v4l2_subdev *sd)
{
	...
	if (!it6616_match_dv_timings(&it6616->timings, &timings)) {
		...
		it6616_s_dv_timings(sd, 0, &timings);
...
static int it6616_isr(struct v4l2_subdev *sd, u32 status, bool *handled)
{
	...
	if (tx_5v_power_present(sd)) {
		...
		it6616_s_dv_timings(sd, 0, &default_timing);
		it6616->nosignal = true;
		v4l2_dbg(1, debug, sd, "HDMI unplug\n");
	}
...
static int it6616_g_input_status(struct v4l2_subdev *sd, u32 *status)
{
	...
	*status |= no_signal(sd) ? V4L2_IN_ST_NO_SIGNAL : 0;
	...
	v4l2_dbg(1, debug, sd, "get input status: 0x%x\n", *status);
				 ...
				 struct v4l2_dv_timings *timings)
{
	...
	if (v4l2_match_dv_timings(&it6616->timings, timings, 0, false)) {
		v4l2_dbg(1, debug, sd, "set_dv_timings: no change\n");
		return 0;
	}

	if (!v4l2_valid_dv_timings(timings,
				&it6616_timings_cap, NULL, NULL)) {
		v4l2_dbg(1, debug, sd, "set_dv_timings: timings out of range\n");
		return -ERANGE;
	}
				...
				struct v4l2_dv_timings *timings)
{
	...
	*timings = it6616->timings;
	...
	if (!v4l2_valid_dv_timings(timings, &it6616_timings_cap, NULL,
				NULL)) {
		v4l2_dbg(1, debug, sd, "query_dv_timings: timings out of range\n");
...
static int it6616_s_stream(struct v4l2_subdev *sd, int on)
{
	...
	dev_info(&client->dev, "set_stream: set to %s, %dx%d@%d\n",
				on ? "on" : "off",
	...
	if (on) {
		...
		if (ret) {
			dev_err(it6616->dev, "Failed to start it6616 stream: %d\n", ret);
			...
			struct v4l2_subdev_format *format)
{
	...
	v4l2_dbg(1, debug, sd, "get_fmt: fmt code:%d, w:%d, h:%d, field code:%d\n",
...
static int it6616_get_edid(struct v4l2_subdev *sd, struct v4l2_edid *edid)
{
	struct it6616 *it6616 = to_it6616(sd);
	char buf[256];
...
static const struct v4l2_ctrl_config it6616_ctrl_audio_sampling_rate = {
	.id = V4L2_CID_IT6616_AUDIO_SAMPLING_RATE,
...
static const struct v4l2_ctrl_config it6616_ctrl_audio_present = {
	.id = V4L2_CID_IT6616_AUDIO_PRESENT,
	.name = "Audio present",
...
static int it6616_init_v4l2_ctrls(struct it6616 *it6616)
{
	...
	if (it6616->hdl.error) {
		...
		v4l2_err(sd, "cfg v4l2 ctrls failed: %d\n", ret);
		return ret;
	}
	...
	if (it6616_update_controls(sd)) {
		ret = -ENODEV;
		v4l2_err(sd, "update v4l2 ctrls failed: %d\n", ret);
		return ret;
	}
...
static int it6616_probe(struct i2c_client *client)
{
	...
	if (err) {
		dev_err(dev, "it6616_parse_of failed: %d\n", err);
		return err;
	}
	it6616_reset(it6616);
	...
	if (err) {
		dev_err(dev, "it6616_initial failed: %d\n", err);
		err = -ENODEV;
		goto unregister_edid_i2c;
	}
	...
	if (err < 0) {
		v4l2_err(sd, "media entity init failed: %d\n", err);
		goto err_free_hdl;
	}
#endif
	...
	if (err < 0) {
		v4l2_err(sd, "v4l2 register subdev failed: %d\n", err);
	...
	if (err) {
		v4l2_err(sd, "v4l2 ctrl handler setup failed: %d\n", err);
		goto err_work_queues;
	}

	err = device_create_file(dev, &dev_attr_edid_ram);
	err |= device_create_file(dev, &dev_attr_mipi_reg);
	err |= device_create_file(dev, &dev_attr_hdmi_reg);
	if (err)
...
#endif
	v4l2_ctrl_handler_free(&it6616->hdl);
	device_remove_file(it6616->dev, &dev_attr_edid_ram);
	device_remove_file(it6616->dev, &dev_attr_mipi_reg);
	device_remove_file(it6616->dev, &dev_attr_hdmi_reg);
	mutex_destroy(&it6616->confctl_mutex);

...
{
	struct rk628_bt1120 *bt1120 = timer_container_of(bt1120, t, timer);
		...
		struct v4l2_subdev_format *format)
{
	...
	int ret = rk628_bt1120_get_fmt(sd, state, format);
...
static const struct v4l2_ctrl_config rk628_bt1120_ctrl_audio_sampling_rate = {
	.id = V4L2_CID_AUDIO_COMPRESSION_ATTACK_TIME,
...
static void rk628_bt1120_remove(struct i2c_client *client)
{
	...
	if (!bt1120->hdmirx_irq) {
		timer_delete_sync(&bt1120->timer);
		flush_work(&bt1120->work_i2c_poll);
	}

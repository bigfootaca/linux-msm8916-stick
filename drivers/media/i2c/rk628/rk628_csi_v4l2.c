...
static int rk628_csi_s_dv_timings(struct v4l2_subdev *sd,
				 unsigned int pad, struct v4l2_dv_timings *timings);
...
{
	struct rk628_csi *csi = timer_container_of(csi, t, timer);
		...
		struct v4l2_subdev_format *format)
{
	...
	int ret = rk628_csi_get_fmt(sd, state, format);
...
static ssize_t reset_stream_store(struct device *dev,
				  ...
				  const char *buf, size_t count)
{
	...
	rk628_csi_reset_streaming(&csi->sd, 0);
	usleep_range(5000, 5500);
	rk628_csi_reset_streaming(&csi->sd, 1);
...
static int rk628_csi_probe(struct i2c_client *client)
{
	...
	err = device_add_groups(dev, rk628_groups);
	if (err < 0)
		goto err_hdl;

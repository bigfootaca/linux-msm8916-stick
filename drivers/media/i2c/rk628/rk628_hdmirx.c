...
static int hdcp_load_keys_cb(struct rk628 *rk628, struct rk628_hdcp *hdcp)
{
	...
	ret = request_firmware(&firm, "rockchip/hdcp1x_key.bin", rk628->dev);
	if (ret < 0 || !firm || firm->size < (HDCP_KEY_SIZE + HDCP_KEY_SEED_SIZE)) {
		dev_warn(rk628->dev, "HDCP: failed to load keys\n");
		kfree(hdcp->keys);
		hdcp->keys = NULL;
		kfree(hdcp->seeds);
		hdcp->seeds = NULL;
